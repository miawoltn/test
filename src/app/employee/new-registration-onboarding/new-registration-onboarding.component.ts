import { recruitmentUrl } from './../../recruitment/recruitment-url';
...
@Component({
  selector: 'app-new-registration-onboarding',
  templateUrl: './new-registration-onboarding.component.html',
  styleUrls: ['./new-registration-onboarding.component.scss'],
})
export class NewRegistrationOnboardingComponent implements OnInit {
  ...
  loadItem(id) {
    this.http.get(recruitmentUrl.applicants.get + id).subscribe(
      (success) => {
        // this.obj = success.data;
        const data = success.data.applicant;
        console.log(data);
        // filter state
        this.loadStates(data.NationalityID);

        let emp = {
          // ApplicantID: data.ApplicantID,
          FirstName: data.FirsName,
          SurName: data.LastName,
          FullName: data.Applicant,
        ...
        emp.ApplicantID = data.ApplicantID;
        this.obj = emp;

        this.isEditable = true;
        this.operation = 'On-Board Employee';
      },
      (error) => {
        console.log(error);
        swal.fire('Error', error.message || 'Some Error Occured', 'error');
