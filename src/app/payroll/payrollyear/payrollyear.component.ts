import { settingsUrl } from 'src/app/settings/settings-url';
import { Component, OnInit, ViewChild } from "@angular/core";
import { HttpService } from "src/app/services/http.service";
import { payrollUrl } from "../payroll-url";
import swal from "sweetalert2";
@Component({
  selector: "app-payrollyear",
  templateUrl: "./payrollyear.component.html",
  styleUrls: ["./payrollyear.component.scss"],
})
export class PayrollyearComponent implements OnInit {
  displayedColumns: string[] = [
    "YeardID",
    "YearName",
    "DateFrom",
    "DateTo",
    "YearID",
  ];
  data = [];
  obj = {
    YeardID: null,
    YearName: null,
    DateFrom: null,
    DateTo: null,
    YearID: null,
  };
  displayForm = false;
  operation = "Add";
  title = "Payroll Year";
  parent = "Payroll";
  year: any;

  constructor(protected http: HttpService) {
    this.loadItem = this.loadItem.bind(this);
  }

  ngOnInit() {
    this.http.get(payrollUrl.payrollyear.list).subscribe(
      (res) => {
        this.data = res.data;
      },
      (err) => {
        console.log(err);
      }
    );

    this.http.get(settingsUrl.year.list).subscribe(
      (res) => {
        this.year = res.data;
      },
      (err) => {
        console.log(err);
      }
    );
  }

  showForm() {
    this.displayForm = true;
    window.scrollTo({ top: 0, left: 0, behavior: "smooth" });
  }

  hideForm() {
    this.displayForm = false;
  }

  addItem() {
    this.resetForm();
    this.showForm();
  }

  processForm() {
    let url = payrollUrl.payrollyear.add;
    if (this.operation == "Update") url = payrollUrl.payrollyear.update;
    this.save(url);
  }

  save(url) {
    this.http.post(url, this.obj).subscribe(
      (res) => {
        this.data = res.data;
        swal.fire("Process Complete", res.message, "success");
        this.resetForm();
      },
      (err) => {
        console.log(err);
        swal.fire("Process Unsuccessful", err.error.message, "error");
      }
    );
  }

  loadItem(id) {
    this.obj = this.data.find((item) => item.id === id.row.data.id);
    this.operation = "Update";
    this.showForm();
  }

  resetForm() {
    this.obj = {
      YeardID: null,
      YearName: null,
      DateFrom: null,
      DateTo: null,
      YearID: null,
    };
    this.hideForm();
    this.operation = "Add";
  }
}
