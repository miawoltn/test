...
@Component({
  selector: 'app-periodtype',
  templateUrl: './periodtype.component.html',
  styleUrls: ['./periodtype.component.scss']
})
export class PeriodtypeComponent implements OnInit {
    
	data = [];
    obj={ periodType_name : null };
    displayForm = false;
    operation = "Add";
	title="Periodtype";
    ...
    processForm()
    {
      let url = periodsUrl.periodtype.add;
      if(this.operation=="Update"){
        url = periodsUrl.periodtype.update;
		this.save(url);
	  }
	  else
		this.add(url);
